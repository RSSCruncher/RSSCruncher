...
class FeedController extends FOSRestController {
    ...
    public function getFeedsAction(Request $request, ParamFetcherInterface $paramFetcher)
    {
        ...
        return $this->container->get('arthur_hoaro_feeds_api.feed.handler')->all($limit, $offset);
    }
     ...
     * @ApiDoc(
     *   resource = true,
     *   description = "Gets a IFeed for a given id",
     ...
     *   input = "ArthurHoaro\FeedsApiBundle\Form\FeedType",
     *   statusCodes = {
     *     200 = "Returned when successful",
    ...
    public function postFeedAction(Request $request)
    {
        try {
            ...
            $newFeed = $this->container->get('arthur_hoaro_feeds_api.feed.handler')->post(
                $request->request->all()
            );
    ...
    {
        try {
            if (!($feed = $this->container->get('arthur_hoaro_feeds_api.feed.handler')->get($id))) {
                $statusCode = Codes::HTTP_CREATED;
                $feed = $this->container->get('arthur_hoaro_feeds_api.feed.handler')->post(
                    $request->request->all()
                );
            } else {
                $statusCode = Codes::HTTP_NO_CONTENT;
                $feed = $this->container->get('arthur_hoaro_feeds_api.feed.handler')->put(
                    $feed,
                    $request->request->all()
                );
            }
    ...
    {
        try {
            $feed = $this->container->get('arthur_hoaro_feeds_api.feed.handler')->patch(
                $this->getOr404($id),
                $request->request->all()
            );
    ...
    /**
     * Fetch a IFeed or throw an 404 Exception.
     *
     * @param mixed $id
     *
     * @return \ArthurHoaro\FeedsApiBundle\Model\IFeed
     *
